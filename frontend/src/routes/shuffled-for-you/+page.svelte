<script lang="ts">
  ...
  import { shuffledForYou, likePoint, unlikePoint, dislikePoint } from '$lib/api/endpoints';
  ...
  async function refresh() {
    ...
    nextPage = 0;
    try {
      const res = await shuffledForYou({ limit: PAGE });
  ...
  async function loadMore(signal?: AbortSignal) {
    ...
    loading = true;
    try {
      const res = await shuffledForYou({
        page: nextPage,
        limit: PAGE,
        signal,
      });
