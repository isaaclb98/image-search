"""search/routers/shuffled_for_you.py — /api/shuffled-for-you/feed.

GET /api/shuffled-for-you/feed?top_pct=1&limit=30&page=0:
...
------------
    ...
    limit : int, default 30, range [1, SHUFFLED_MAX_LIMIT].
...
from search.shuffled_for_you import rank as rank_shuffled
...
def build_shuffled_for_you_router(
    *,
    index_db: Any,
    qdrant: Any,
    cfg: Any,
) -> APIRouter:
    """Build the /shuffled-for-you router with the live dependencies.
    ...
    `search.shuffled_for_you.invalidate_pool_cache()`.
    """
    router = APIRouter()
    _web_ui_url = cfg.web_ui_url

    @router.get("/api/shuffled-for-you/feed", response_model=SearchResponse)
    async def shuffled_for_you_feed(
        ...
        limit: Annotated[
            int,
            Query(ge=1, le=SHUFFLED_MAX_LIMIT, description="Photos per page."),
        ] = 30,
        page: Annotated[
            int,
        ...
        try:
            ...
            logger.exception("shuffled-for-you: failed to read fav/dis ids")
        ...
        try:
            ...
            logger.exception("shuffled-for-you: rank failed")
