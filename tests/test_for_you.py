"""
tests/test_for_you.py — Backend tests for the For You feed (formerly
Shuffled For You — round 22; the feature replaced the prior
recommend-with-diversity-rerank pipeline on the same /api/for-you/feed
URL on round 33).

Three test groups:
1. Pure math (explore_pool_size): smoke + boundary cases.
2. Pool builder (build_for_you_pool): recommend() path, cold-start zero-vector
   path, cache hit/miss/TTL/invalidation behaviour.
3. Router (build_for_you_router): happy path with mocked
...
# ===========================================================================
# 2. Pool builder — build_for_you_pool
# ===========================================================================
...
class TestBuildPool:
    ...
    def test_cache_miss_after_invalidate(self) -> None:
        """After invalidate_for_you_cache(), the next call rebuilds."""
        qdrant = MagicMock()
    ...
    def test_different_top_pct_yields_different_cache_key(self) -> None:
        ...
        build_for_you_pool(fav_ids=["fav"], dis_ids=[], qdrant=qdrant, index_db=index_db, top_pct=1.0)
        build_for_you_pool(fav_ids=["fav"], dis_ids=[], qdrant=qdrant, index_db=index_db, top_pct=2.0)
        assert qdrant.recommend.call_count == 2
    ...
    def test_different_fav_ids_yields_different_cache_key(self) -> None:
        ...
        build_for_you_pool(fav_ids=["fav-1"], dis_ids=[], qdrant=qdrant, index_db=index_db, top_pct=1.0)
        build_for_you_pool(fav_ids=["fav-2"], dis_ids=[], qdrant=qdrant, index_db=index_db, top_pct=1.0)
        assert qdrant.recommend.call_count == 2
...
# ===========================================================================
# 3. rank_for_you() — page slicing + retrieve_batch
# ===========================================================================
...
class TestRank:
    ...
    def test_returns_page_sliced_from_pool(self) -> None:
        ...
        # Mock retrieve_batch to return ALL hits; rank_for_you() will
        # re-order by the shuffled pool order.
        qdrant.retrieve_batch.return_value = list(pool)

        hits, pool_total, has_more = rank_for_you(
            fav_ids=["fav"],
            dis_ids=[],
            qdrant=qdrant,
            index_db=index_db,
    ...
    def test_has_more_false_on_last_page(self) -> None:
        ...
        _, pool_total, has_more = rank_for_you(
            fav_ids=["fav"],
            dis_ids=[],
            qdrant=qdrant,
            index_db=index_db,
...
class TestRouter:
    ...
    def test_happy_path_returns_search_response_shape(self) -> None:
        ...
        with TestClient(app) as client:
            response = client.get("/api/for-you/feed")
    ...
    def test_top_pct_zero_rejected(self) -> None:
        ...
        with TestClient(app) as client:
            response = client.get("/api/for-you/feed?top_pct=0.0")
    ...
    def test_top_pct_over_100_rejected(self) -> None:
        ...
        app, _, _ = self._build_router()
        with TestClient(app) as client:
            response = client.get("/api/for-you/feed?top_pct=101")
        assert response.status_code == 422
    ...
    def test_limit_zero_rejected(self) -> None:
        ...
        app, _, _ = self._build_router()
        with TestClient(app) as client:
            response = client.get("/api/for-you/feed?limit=0")
        assert response.status_code == 422
    ...
    def test_page_negative_rejected(self) -> None:
        ...
        app, _, _ = self._build_router()
        with TestClient(app) as client:
            response = client.get("/api/for-you/feed?page=-1")
        assert response.status_code == 422
    ...
    def test_response_results_have_required_fields(self) -> None:
        ...
        with TestClient(app) as client:
            response = client.get("/api/for-you/feed?limit=2")
        body = response.json()
        ids_in_response = {r["id"] for r in body["results"]}
        # Pool is randomly shuffled — assert both ids are returned
        # (modulo the auto-fill path), not specific order.
        assert {"id-0", "id-1"} <= ids_in_response
        # First result carries the SearchResult-shape fields.
        first = body["results"][0]
        assert "url" in first
        assert first["url"].startswith("http://test:1234/photo/")
        assert first["url"].endswith("/raw")
    ...
    def test_session_total_reflects_pool_size(self) -> None:
        ...
        with TestClient(app) as client:
            response = client.get("/api/for-you/feed?top_pct=2&limit=5")
        body = response.json()
    ...
    def test_offset_echoes_input(self) -> None:
        ...
        with TestClient(app) as client:
            response = client.get("/api/for-you/feed?page=2&limit=10")
        body = response.json()
        assert body["offset"] == 20
        assert body["limit"] == 10

    def test_invalidate_callback_clears_cache(self) -> None:
        """Calling invalidate_for_you_cache() forces the next request to rebuild."""
        ...
        with TestClient(app) as client:
            ...
            client.get("/api/for-you/feed?limit=5")
