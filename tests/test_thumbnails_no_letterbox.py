...
def _isolated_thumb_dir(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("THUMBNAIL_DIR", str(TEST_THUMB_DIR))
    TEST_THUMB_DIR.mkdir(parents=True, exist_ok=True)
    # Clear any prior test runs.
    for old in TEST_THUMB_DIR.glob("test-round30-*"):
        old.unlink()
...
# Portrait source — the same aespa/Ningning photo we used to verify the
# bug visually. Test against multiple sources to catch the symmetric
# landscape case too.
PORTRAIT_SRC = Path(
...
def test_letterboxed_input_produces_bars() -> None:
    ...
    # Sanity: load() should return the letterboxed (model-input) size,
    # which is square. This is the property the rest of the bug depends
    # on — if it ever stops being square, the thumbnail bug goes away.
...
def test_original_input_produces_clean_thumbnail() -> None:
    ...
    # Sanity: the original must NOT be square — the letterbox step is
    # what made it square. If this ever stops being true, the thumbnail
    # bug is moot.
